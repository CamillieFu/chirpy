...
// Wrote a function to send stored email & api token to the active tab every 5 seconds if the url has twitter
chrome.runtime.onMessage.addListener(function (request, sender, sendResponse) {
...
function sendAuth_userMsg() {
  chrome.storage.sync.get(["user", "api_token"], function (data) {
